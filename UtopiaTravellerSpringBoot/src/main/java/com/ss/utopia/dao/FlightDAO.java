...
//			obj.setSeatPrice(rs.getFloat("seat_price"));
	...
	public Integer create(Flight obj) throws SQLException, ClassNotFoundException {
		String query = "insert into flight (route_id, airplane_id, departure_time, arrival_time, reserved_seats, seat_price, id) values (?,?,?,?,?,?,?)";
//		KeyHolder keyHolder = new GeneratedKeyHolder();
		jdbcTemplate.update(query, obj.getRoute().getId(), obj.getPlane().getId(), obj.getDepartureTime(), obj.getArrivalTime(), obj.getId());
		return null;
...
//		obj.setId(key);
//		return key;
		//Due to teams design choice the id is NOT auto-incrementing and must instead be passed in
	...
	public void update(Flight obj) throws SQLException, ClassNotFoundException {
		jdbcTemplate.update(
				"update flight set route_id = ?, airplane_id = ?, departure_time = ?, arrival_time = ?, reserved_seats = ?, seat_price = ? where id = ?",
				obj.getRoute().getId(), obj.getPlane().getId(), obj.getDepartureTime(), obj.getArrivalTime(),
				obj.getId());
